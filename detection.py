...
def capture(Sensibilité,Pixelisation, sensibilite_vecteur,mode):
        ...
        """
        i = False#i est une variable bouléenne qui sui si le mouvement est corect
        ...
        video = cv2.VideoCapture(0) #permet de lancer une capture vidéo, 0 corespond à l'id de la caméra 0 : la première caméra du pc. Dans le cas ou l'ont veux changer de caméra il faut changer le 0 par l'id de la caméra-
        taille = (int(video.get(cv2.CAP_PROP_FRAME_WIDTH)), int(video.get(cv2.CAP_PROP_FRAME_HEIGHT))) #on récupére mes dimensions en px de la camera dans un tuple
        ...
        temps_depuis_derniere_coord=0#variable qui a le time où les dernier vecteur on été calculé
        l_coord=[]#liste des position sucessive de la main
        if modedefonctionement.get()==0:#mode0 ==mode powerpoint , mode1==mode musique
            #définition des mouvement à l'aide de la Class Deplaccement de vecteur.py pour powerpoint
        ...
        while True:
            ret, frame = video.read()# on récupère l'image de la caméra: frame est une liste de liste de tuple (b,g,r (c'est du rgb mais pas dans le même sens parceque open cv voulez se sentir diffèrent) d'entier codé sur 8 bit(255)
            masque = back.apply(frame,None,0)#on aplique l'algorythme de substraction de l'arrière plan, cela renvoir un masque (0 ou 1) pour chaque pixel, un 1 montre un pixel qui à changer signicativement par rapport au fond
            masque = cv2.erode(masque, element, iterations=1)#on applique un effet d'érosion sur le masque, on prend des carrée de élement(15*15 par exemple) sur l'image est on calcule,une moyenne locale puis on définit la valeur de chaque pixel du carrée à la moyenne des valeur arondie à l'entier le plus prés, cela permet de se débarrasser des pixel de bruit qui parasitent l'écran
            x, y, largeur, longueur = cv2.boundingRect(masque)#on utilise la fonction bouding rect qui renvoie les coordonnés x,y du coin supérieur ainsi que la largeur et la longueur du plus petit rectangle contenant tous les pixel de valeur 1, on obtient un rectangle qui fait le "contour" des pixel qui ne font pas partis du fond
            cv2.rectangle(frame,(x,y),(x+largeur,y+longueur),(0,255,0),2)#on afiche un rectangle sur le retour caméra avec les coordonnées precedent. Malheuresement open CV n'est JAMAIS consistant donc ici il faut donner les coordonnées du point supérieur gauche et inférieur droit alors que la fonction boundringrec renvoie un rectangle avec sa largeur et sa longueur, Bref trop de soufrance
            gauche = (x, argmax(masque[:, x])) #alors en fait on sait que sur l'axe x=x il y a le pixel le plus à gauche qui a des coordonnés de la forme (x,?), pour ? on va prendre le premier 1 de l  collone on a ainsi les coordonnés du pixel le plus à gauche de la main
            droite = (x + largeur - 1, argmax(masque[:, x + largeur - 1])) #pareille pour le pixel le plus à droite (le -1 est la car x+largeur donne une valeur avec 1 de trop pour les cordonnés x de la droite de droite
            haut = (argmax(masque[y, :]), y)#pareil
            bas = (argmax(masque[y + longueur - 1, :]), y + longueur - 1)#extrement similaire
            cv2.circle(frame, gauche, 8, (255, 50, 0), -1) #on place un point sur lécran au coordonné du pixel le plus à gauche. (255,50,0) désigne la couleur (en rgb ici pas en bgr parceque pourquoi pas) -1 est la largeur des bord du cercle,-1 permet de remplir le cercle afin d'avoir un point
            ...
            if i is True :#si le mouvement est corecte, on affiche les "vecteur", en reliant les coordonnées sucecive de la main stockés dans lcord
            ...
            cv2.imshow("Cam", masque)#on renvoie le masque obrtenu
            #Calcul des vecteurs
            if time.time()-temps_depuis_derniere_coord>0.25:#on calcule les vecteur tous les 0.25 sec
                if len(l_coord)==4:#si il y 4 coord dans l_coord
                    ...
                    i = False #pour l'instant les vecteur ne sont pas "valide"
                    if v03.module > math.sqrt(taille[0]**2 + taille[1]**2) * sensibilite_vecteur / 100 :#premier test est ce que le vecteur est asseze grand à par rapport à un  % de  la diagonale de l'écran
                            if v01.module > v03.module*0.1 and v12.module > v03.module*0.1 and v23.module > v03.module*0.1 : #est ce que les vecteur intermédiaire ne sont pas trop petit (mouvement brutale) ou fausse détection
                                for mv in mouvement:#pour chaque type de mouvement du mode-
                                    if {1:v03.argument<mv.a1[0] or v03.argument>mv.a1[1], 0:v03.argument<mv.a1[0] and v03.argument>mv.a1[1]}[mv.andor]:#on vérifie que le grand vecteur à un argument dans l'angle du mouvement par exemple entre pi/4 et 3pi/4 pour un mouvement par le haut
                ...
                else:#si il y a moins de 4 coordonée dans la liste == début de programmes (moins de 1 sec depuis le lancement)
                    l_coord.append(gauche)#on rajoute les coordonnées et reset le timer,on enlève rien de liste car elle n'a pas plus que 4 point
                    temps_depuis_derniere_coord=time.time()

            if cv2.waitKey(1) & 0xFF == ord('q') :#si la touche q tu clavier est appuyé
                break#on sort de while
        video.release()#on arrète le capture vidéo
        cv2.destroyAllWindows() #et on ferme tout les fenètres
...
def ChoisirCouleurEcran () :
    ...
    tm = time.time()
    while time.time()-tm <= 5:#tant que moins de 5 sec sont passès
        ret, frame = video.read()#on récupère l'image
        frame = cv2.erode(frame, ones((5, 5), uint8), iterations=1)#on érode l'image pour perdre les détail et rendre la selection de couleur plus simple (sinon on risque de selectionné des pixels de bruit, noir ou blanc), faire la moyenne locale des pixel permet de lisser l'image et les couleurs rendant moins fastidieux la recherche de pixel
    ...
    cv2.destroyAllWindows()#Et on ferme les fenetre open cv
    c=frame[coord[0]][coord[1]]#on récupère la couleur du pixel du milieu
    c[0],c[2]=c[2],c[0]#on passe de bgr à rgb car Open CV à décidé de fonctionner en bgr pour aucune raison
...
def suiviMain(couleur):
    ...
    couleur=couleur[0][0]#on récupère la couleur qui était stockés dans une image Open Cv d'un pixel, on récupère don le pixel de coordonnés 0,0 de l'image à un pixel
    ...
    #ainsi pour que 2 couleur soit ""proche" il faut seulement que le H soit extrement similaire, la saturation des deux couelur doit être "proche" et il suffit que le V ne soit pas trop bas (car sinon c'est juste du noir)
    #le HSV de Open est diffèrent du normale, il garde les inforation dans des pixel valide en rgb dans H,S,V sont des entier codés sur 8 bit(255) alors que le HSV clasique est en %
    ...
    while 1:
        ...
        hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)#on âss de BGR(rgb d'Open CV) aux HSV: https://fr.wikipedia.org/wiki/Teinte_Saturation_Valeur
        masque = cv2.inRange(hsv, bas, haut)#on creer un masque, les pixel qui sont daans le range couleur basse/haute sont "valide"==1 et les autre ==0
        masque = cv2.erode(masque, ones((50, 50), uint8), iterations=1)#On érode l'image pour enlever des détail (des pixels seuls de la bonne couleur, des défauts de la caméra, une mouche qui vole...)
        ...
        cv2.circle(sortie,(x+largeur//2,y+longueur//2),6,(255,0,0),-1)#on affiche un point au centre du rectangle à la croisé des diagonales
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
    video.release()#si la touche q est appuyé ont ferme open cv
...
def CouleurOK(couleurhex):
    ...
    if len(couleurhex)==6:
        ...
        i=False
    return "#"+couleurhex if i else "#FFFFFF" #on renvoie le hex si il est valide sinon on renvoie #FFFFFFF du blanc
...
###################################
#Note: je ne vais pas comenter tous le Tkinter, c'est long et plutot simple je vais simplement commenter les choses intérréssantes
##################################
...
def SourisTk():
    ...
    """
    #on enlève les élement du mode powerpoint et musique pour afficher les element du mode souris
    slider1.grid_remove()
    slider2.grid_remove()
    slider3.grid_remove()
    l1.grid_remove()
...
def ChoisirCouleur():
    ...
    textecouleur.set(couleur[1])#On met le texte à la couleur choisis
...
modedefonctionement=tkinter.IntVar()#Les Int var sont des variable dynamique si leur contenue change alors les affichage qui dépandent de la variable vont changer ex:si je définis un texte avec une variable et que je change la variable, le texte ne va pas changer . Avec un INTvar le texte change
modedefonctionement.set=0#on initialise avec un setter modedefonctionement à 0 (powerpoint)
textecouleur=tkinter.StringVar()#On creeer un STR (meme principe que INTvar plus haut) qui va afficher le hex de la couleur
...
slider3.set(10)
#Les radio boutons pour changer de mode , ils font changer la variable mode de fonctionement et apellé les fonction qui permettens d'afficher et de faire disparaite les élement concernés
...
labelsouris=tkinter.Entry(Fenetre_tkinter,textvariable=textecouleur)#Le label souris ou il est écrit la couleur
textecouleur.trace_add("write",lambda *args: labelsouris.config(bg=CouleurOK(textecouleur.get())))#Quand le texte est modifié, on change la couleur du background, on, passe le texte  de CouleurOK pour embécher les erreurs quand on commence à taper une couleur ainsi #56 n'est pas un hex valide, la fonction va ranvoyer du blanc
boutoncouleurecran= tkinter.Button(Fenetre_tkinter,text="Choisir la couleur depuis la caméra",command=ChoisirCouleurEcran)

#On affiche les éléments "de base" qui reste toujours à l'écran
...
#On appelle PowerPointTk pour afficher les élement relatif au mode de fonctionement 1
PowerPointTk()
